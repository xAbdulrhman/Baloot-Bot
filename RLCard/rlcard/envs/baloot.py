...
from rlcard.games.baloot import Game

from rlcard.games.baloot.game import BalootGame
from rlcard.games.baloot.utils.action_event import ActionEvent
from rlcard.games.baloot.utils.baloot_card import BalootCard
from rlcard.games.baloot.utils.move import CallMove, PlayCardMove
...
class BalootEnv(Env):
    ...
    def __init__(self, config):
        ...
        self.BalootPayoffDelegate = DefaultBalootPayoffDelegate()
        self.BalootStateExtractor = DefaultBalootStateExtractor()
        state_shape_size = self.BalootStateExtractor.get_state_shape_size()
    ...
    def get_payoffs(self):
        ...
        '''
        return self.BalootPayoffDelegate.get_payoffs(game=self.game)
    ...
    def _extract_state(self, state):  # wch: don't use state 211126
        ...
        '''
        return self.BalootStateExtractor.extract_state(game=self.game)
...
class BalootPayoffDelegate(object):

    def get_payoffs(self, game: BalootGame):
...
class DefaultBalootPayoffDelegate(BalootPayoffDelegate):
...
class BalootStateExtractor(object):  # interface

    def get_state_shape_size(self) -> int:
        raise NotImplementedError

    def extract_state(self, game: BalootGame):
        ''' Extract useful information from state for RL. Must be implemented in the child class.

        Args:
            game (balootGame): The game
    ...
    @staticmethod
    def get_legal_actions(game: BalootGame):
        ''' Get all legal actions for current state.
...
class DefaultBalootStateExtractor(BalootStateExtractor):
    ...
    def get_state_shape_size(self) -> int:
        state_shape_size = 0
        state_shape_size += 4 * 32  # hands_rep_size
        state_shape_size += 4 * 32  # trick_rep_size
        state_shape_size += 32  # hidden_cards_rep_size
    ...
    def extract_state(self, game: BalootGame):
        ...
        hands_rep = [np.zeros(32, dtype=int) for _ in range(4)]
        if not game.is_over():
        ...
        trick_pile_rep = [np.zeros(32, dtype=int) for _ in range(4)]
        ...
        hidden_cards_rep = np.zeros(32, dtype=int)
        if not game.is_over():
            if game.round.is_bidding_over():
        ...
        if game.round.is_bidding_over() and not game.is_over() and game.round.play_card_count == 0:
            ...
            if contract_bid_move:
                ...
                bid_suit_index = 4 if not bid_suit else BalootCard.suits.index(bid_suit)
